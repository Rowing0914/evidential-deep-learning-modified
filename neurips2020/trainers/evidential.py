...
class Evidential:
    def __init__(self, model, args, dataset="", learning_rate=1e-3, lam=0.0, epsilon=1e-2, maxi_rate=1e-4, tag=""):
        ...
        self.logger = wandb_logger(args=vars(args))
    ...
    def train(self, x_train, y_train, x_test, y_test, y_scale, batch_size=128, iters=10000, verbose=False):
        tic = time.time()
        for self.iter in range(iters):
            ...
            if self.iter % 10 == 0:
                self.logger.add_scalar("train/mse", tf.reduce_mean(edl.losses.MSE(y_input_batch, y_hat)), step=self.iter)
            ...
            if self.iter % 100 == 0:
                ...
                self.logger.add_scalar("test/mse", tf.reduce_mean(edl.losses.MSE(y_test_batch, mu)), step=self.iter)
                ...
                if self.running_rmse < self.min_rmse:
                    ...
                    if self.args.save: self.save(f"model_rmse_{self.iter}")
                ...
                if self.running_nll < self.min_nll:
                    ...
                    if self.args.save: self.save(f"model_nll_{self.iter}")
                ...
                if self.running_vloss < self.min_vloss:
                    ...
                    if self.args.save: self.save(f"model_vloss_{self.iter}")
